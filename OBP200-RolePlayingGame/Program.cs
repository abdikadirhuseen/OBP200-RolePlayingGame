...
class Program
{
    ...
    static void StartNewGame()
    {
        ...
        Player = new Player(name, selectedClass, hp, maxhp, atk, def, gold, potions);
        ...
        Console.WriteLine($"Välkommen, {Player.Name} the {Player.Class}!");
        ShowStatus();
    }
    ...
    {
        return Player.PlayerClass.CalculateAttackDamage(Player, enemyDef, Rng);
    }

    static int UseClassSpecial(int enemyDef, bool vsBoss)
    {
        return Player.PlayerClass.UseSpecial(Player, enemyDef, vsBoss, Rng);
    }
    ...
    {
        if (Player.UsePotion(out int healedAmount))
        {
            Console.WriteLine($"Du dricker en dryck och återfår {healedAmount} HP.");
        }
        else
        {
    ...
    {
        return Player.PlayerClass.TryRunAway(Rng);
    }
    ...
    static void MaybeLevelUp()
    {
        while (true)
        {
            ...
            Player.LevelUp();
            Player.PlayerClass.ApplyLevelUp(Player);
            Player.FullHeal();
    ...
    static void SellMinorGems()
    {
        ...
        var updatedItems = items.Where(x => x != "Minor Gem").ToList();
        Player.SetInventoryItems(updatedItems);
    ...
    static void ShowStatus()
    {
        ...
        if (items.Count > 0)
        {
            Console.WriteLine($"Väska: {string.Join("; ", items)}");
        }
    }
